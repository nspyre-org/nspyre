...
class TaskVsLine(Spyrelet):
    ...
    def main(self, lines, f, ch1, ch2, rf_power, time_per_point=0.5, iterations=100):
        ...
        for i in self.progress(iterator):
            for line in self.progress(lines):
    ...
    def initialize(self, lines, f, ch1, ch2, rf_power, time_per_point=0.5, iterations=100):
    ...
    def finalize(self, lines, f, ch1, ch2, rf_power, time_per_point=0.5, iterations=100):
    ...
    def avg(df, cache):
        g = df.groupby('line')
        lines = g.ch1.mean().index
        return {'ch1':[lines, g.ch1.mean()],'ch2':[lines, g.ch2.mean()]}

    @Plot1D
    def latest(df, cache):
        latest = df[df.i == df.i.max()]
        return {'ch1':[latest.line, latest.ch1],'ch2':[latest.line, latest.ch2]}

    @Plot1D
    def diff_avg(df, cache):
        g = df.groupby('line')
        lines = g.ch1.mean().index
        return {'ch1-ch2':[lines, g.ch1.mean()-g.ch2.mean()]}

    @Plot1D
    def diff_latest(df, cache):
        latest = df[df.i == df.i.max()]
        return {'ch1-ch2':[latest.line, latest.ch1-latest.ch2]}

    @PlotFormatInit(LinePlotWidget, ['avg', 'latest', 'diff_avg', 'diff_latest'])
    def init_plot(plot):
        plot.xlabel = 'AWG Line'
        plot.ylabel = 'Signal'