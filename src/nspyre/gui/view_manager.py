...
class ViewManagerWindow(QMainWindow):
    def __init__(
        ...
        **kwargs,
        ...
        vertical_splitter = Splitter(
            main_w=horizontal_splitter,
    ...
    def update_plot(self, col_name, row):
        ...
        for name, view in self.views[col_name].items():
            if (
                col_name in self.db.get_df('Register').index
                and 'cache' in self.db.get_df('Register').loc[col_name]
                and not self.db.get_df('Register').loc[col_name]['cache'] is np.nan
            ):
    ...
    def add_col(self, col_name, try_update=True):
        ...
        self.default_color = top.background(
            0
        )  # This is used to remember the default color when instantiating (to restore in update_color)
        ...
        for name, view in spyrelet_views.views.items():
            if view.type == '1D':
                self.views[col_name][name] = LinePlotView(
                    view, self.common_lineplotwidget
                )
            elif view.type == '2D':
                self.views[col_name][name] = HeatmapPlotView(
                    view, self.common_heatmapplotwidget
                )
            self.items[col_name][name] = QtWidgets.QTreeWidgetItem(
                0
        ...
        for i in range(100):
            ...
            if not name in self.views[col_name]:
                if (
                    ...
                    cache = custom_decode(
                        self.db.get_df('Register').loc[col_name]['cache']
                    )
                else:
                    cache = {}
                self.views[col_name][name] = CustomView(
                    self.code_editor,
                    self.common_lineplotwidget,
                    self.common_heatmapplotwidget,
                    self.plot_layout,
                    self.db.get_df(col_name),
                    cache,
                )
    ...
    def del_col(self, col_name):
        ...
        self.tree.takeTopLevelItem(
            self.tree.indexOfTopLevelItem(self.items[col_name]['__top'])
        )
